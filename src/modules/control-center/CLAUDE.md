...
- **Networks, resources & routing peers (draft)** — spec: `specs/draft-networks-resources.md`. Networks drop with a `create-network` change (deployable immediately); `NetworkNode` is draft-aware (NEW badge, resource count incl. draft members via `data.draftNetwork` refs, traffic light counts routing edges, floating "Add Routing Peer" button at 0 routers, full-area target in draft; context menu: Rename / Add Routing Peer / Remove with cascade). Resources drop + open the editor (`DraftResourceEditorModal`: unique name, `ResourceSingleAddressInput` address, network selector offering API + draft networks + inline create, optional groups/description); incomplete resources show an amber "Set up" affordance and stay out of the changeset; complete ones record `create-resource` (edits upsert) and re-run referencing policies (`useDraftNetworkActions.syncDraftResource`). Resources are destinations-only (`handleDraftConnect` hard-rejects resource sources; policy left-handle rejects them). Routing: peer/group → network connect (or "Add Routing Peer": drops a connected Server placeholder + opens install) draws a `floating-straight` "routes" edge and records `create-router` — placeholder-peer routers stay out of the changeset until install (the upgrade sweep records them with the real id). Resource → network connect assigns the parent (membership `simple` edge, one per resource). Deploy order: groups → networks → resources → routers → policies; client ids resolve via `networkClientToId`/`resourceClientToId`, group changes filter `new-` resource ids (membership fans out via the resource's own `groups` field), router body uses live-modal defaults (metric 9999, masquerade, enabled). Review & Deploy shows non-blocking warnings (`getDraftWarnings`): networks with resources but no routers, resources no policy references.
