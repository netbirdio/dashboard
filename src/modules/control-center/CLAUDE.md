...
│   ├── useDraftNetworkActions.ts     → Draft network/resource/router actions: connectRouter, assignResourceToNetwork, saveDraftResource + syncDraftResource, renameDraftNetwork, getNetworkRef
...
- **Networks, resources & routing peers (draft)** — spec: `specs/draft-networks-resources.md`. Draft networks render as a **frame** (border + slightly opaque bg, `NETWORK_FRAME_WIDTH`/`getNetworkFrameHeight` sizing via node style) whose resource nodes live INSIDE as ReactFlow children (`parentId`, relative `getFrameChildPosition`; parents precede children in the array; auto-arrange resolves child connectivity onto the frame and never moves children). Frame header: name + NEW badge + routing-peer traffic light (counts routing edges); floating "Add Routing Peer" button at 0 routers; full-area target; context menu Rename / Add Routing Peer / Remove (cascades changes, children pop out at absolute positions). Dropping a **Resource** auto-creates a frame around it (`create-network` recorded immediately, unique names for both) and opens the editor (`DraftResourceEditorModal`: unique name, `ResourceSingleAddressInput` address, network selector with API + draft networks + inline create — also a frame —, optional groups/description). The draft resource node mirrors the placeholder-peer card (icon box by derived type, NEW badge, dimmed "x.x.x.x" until the address is set); left-click or context-menu Edit reopens the editor; incomplete ones show the amber "Set up" affordance and stay out of the changeset; complete ones record `create-resource` (edits upsert) and re-run referencing policies (`useDraftNetworkActions.syncDraftResource`). Resources carry a single LEFT connect handle: dragging into a policy sets it as the destination (`addResourceToPolicy`; source side rejected), into a network re-assigns the parent (reparented into frames; membership `simple` edge only for non-frame networks); toward peers/groups it's a no-op — destinations only. Complete draft resources drop into groups like peers (their `new-…` id; deploy fans membership out via the resource's own `groups` field). Routing: peer/group → network connect (or "Add Routing Peer": drops a connected Server placeholder + opens install) draws a `floating-straight` "routes" edge and records `create-router` — placeholder-peer routers stay out of the changeset until install (the upgrade sweep records them with the real id). Deploy order: groups → networks → resources → routers → policies; client ids resolve via `networkClientToId`/`resourceClientToId`, group changes filter `new-` resource ids, router body uses live-modal defaults (metric 9999, masquerade, enabled). Review & Deploy shows non-blocking warnings (`getDraftWarnings`): networks with resources but no routers, resources no policy references.
