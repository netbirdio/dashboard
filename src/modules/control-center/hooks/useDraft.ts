...
export function useDraft() {
  ...
  // Live mode always pans, even if the select tool was active in the draft.
  const isSelectMode = isDraft && activeTool === CanvasTool.Select;

  // Also re-runs when draftSession bumps ("New Draft"): the live snapshot must
  // NOT be overwritten with the current draft canvas then.
  useEffect(() => {
    if (isDraft) {
      ...
      // "New Draft" starts empty; the snapshot above still restores on exit.
      ...
      // A rebuild while already drafting must derive from the saved live canvas.
      ...
      // Id indexes: a linear .find per lookup made the build O(nodes²).
      ...
      // React Flow hides unmeasured new nodes for one frame, so draft nodes
      // whose id differs from their live twin adopt that twin's measured size.
      ...
      // The all-networks live view draws no policy nodes, but the frames'
      // policies still belong in the draft.
      liveNodes.forEach((n) => {
      ...
      // Same sort as the live build (disabled first) so the policy and
      // destination columns keep the live vertical order.
      ...
      visiblePolicies?.forEach((policy) => {
        ...
        sources.forEach((source) => {
          ...
          if (!groupId) return;
          // The policy-embedded group snapshot goes stale after a live
          // membership change, so prefer the fresh SWR group.
          const group =
        ...
        // The live GROUP view sorts destinations by name; match the view the
        // draft was entered from so the column order carries over.
        ...
        if (destResource?.id) {
          if (destResource.type === "peer") {
            ...
            if (resource) {
              ...
              // Without a network ref StandaloneResourceNode shows a "No
              // Network" control, wrong for an existing resource.
      ...
      // Carry over entities visible in the live view even without policies so
      // the draft doesn't start empty; a peer covered by a group gets no node.
      ...
      // Live network frames carry into the draft keeping their real id;
      // resources already drawn by the policy pass are reparented into them.
      const carryNetworkFrame = (network: Network) => {
        ...
        // A resource already covered by a group on the canvas and with no node
        // of its own would only become a disconnected duplicate child.
        ...
        // stable partition, so another input order reshuffles untargeted rows.
        ...
        // Seed with the CAPPED grid: an uncapped height makes big frames seed
        // thousands of px tall, which the later shrink leaves as huge gaps.
        ...
        childResources.forEach((r, i) => {
          // Splice + re-push so a reparented child follows its parent in the
          // array.
      ...
      // The live drilled view has no network node on the canvas, so carry the
      // selected network as a frame.
      ...
      // A destination group holding resources of ONE carried network folds into
      // that frame, so the edge reads as "policy → network".
      ...
      allNodes.forEach((node, idx) => {
        ...
        // Same eligibility as a manual drop (canDropGroupIntoNetwork): peer
        // members don't block the fold.
        ...
        if (!frameId) return;
        // Keep the id so its edges follow to the frame; the live overview drew
        // this row under `resource-group-<netId>-<gid>`, so adopt that size.
        allNodes[idx] = {
          ...node,
      ...
      // Re-seed the frame sizes so the grid packing below uses final heights
      // now that the folded group rows count too.
      ...
      // The live networks view never draws peer-only destination groups, so a
      // frames-carrying draft skips them and their edges too.
      ...
      // The shared layout, also used by Auto Arrange, so arranging an untouched
      // draft reproduces the entry layout.
      ...
      // Enter the drill-down in the SAME commit: the pre-applied hidden flags
      // keep the parent view from ever painting.
      if (drilledNetwork) {
        const frameId = `network-${drilledNetwork.id}`;
        // Positions don't matter: the drill-down applies the shared drilled
        // layout.
      ...
      // Fitting an empty draft leaves the viewport in an odd spot, throwing off
      // later drop positions.
      ...
      // Draft-only ids in the snapshot mean this hook remounted mid-draft (hot
      // reload) and snapshotted the draft as "live"; rebuild from data instead.
      ...
      // Leaving draft while drilled into an existing network lands in that
      // network's live view; draft-only networks have no live view.
      ...
      // Backed out of the drill inside the draft: land on the overview, not the
      // stale drilled snapshot.
      if (selectedNetwork) {
        setLayoutInitialized(false);
        setSelectedNetwork("");
        return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- must fire only on the draft mode/session transition
