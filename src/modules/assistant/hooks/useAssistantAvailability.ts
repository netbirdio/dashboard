 ...
 *
 * The assistant is deployed separately from the dashboard, so a configured
 * origin isn't enough — it may be unreachable, not yet rolled out, or unready
 * (its `/readyz` also covers Postgres and JWKS). Everything that
...
export function useAssistantAvailability(): boolean {
  ...
  return Boolean(origin) && reachable;
}
