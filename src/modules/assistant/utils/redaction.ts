 ...
 * NetBird OpenAPI. It lives here because the browser is the only place holding both
 * the real data and the user's trust; the assistant server never sees a raw
 * resource, so it carries no copy of this.
 */
...
// ── identifiers the user types ───────────────────────────────────────────────

/**
 * The same boundary, from the other side.
 *
 * Redaction above is one-way: the model sees `{PEER_1}`, never `eduards-macbook`.
 * That holds until the user *types* "eduards-macbook", at which point the model
 * has a word it has never seen and a list of tokens it can't match it to — so it
 * lists everything and asks, or guesses.
 *
 * So the outgoing message gets the same treatment a tool result does, against the
 * same `Redactor`: `mint` keys on the real value, so a name swapped out of what
 * the user typed and the same name coming back on a `/peers` row produce one
 * token either way round. That is what makes "where is 100.84.175.167?"
 * answerable — the model looks for `{IP_7}` and finds it on a peer row.
 *
 * Two passes, because identifiers come in two flavours: names, which have no
 * structure and must be recognised from the dashboard's loaded data (`catalog`);
 * and addresses and emails, which are structural and match by pattern, so they
 * work for values the dashboard never loaded.
 *
 * Anything missed here is still caught by the server's own PII vault
 * (`guardrails/pii.ts`, minting a separate `{REDACTED_IP_1}` namespace) — but
 * that backstop is per-request and can't line up with account data, which is why
 * the matching has to happen in the browser.
 */
...
const usable = (entry: CatalogEntry) => {
  ...
  return name.length >= MIN_LENGTH && !TOO_GENERIC.has(name.toLowerCase());
};
