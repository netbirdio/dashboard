...
import { countPiiBackstopFailure, countPiiRedaction } from "@/instrumentation/metrics.ts";
...
*/
const analysisCache = new LRUCache<string, PresidioFinding[]>({ max: 500 });
...
export async function analyzeText(text: string): Promise<PresidioFinding[]> {
  ...
  try {
    ...
    analysisCache.set(text, findings);
    return findings;
  } catch (err) {
    countPiiBackstopFailure();
    console.error(
...
/*
  ...
  tool parts carry data the dashboard already redacted. Scrubbing runs in
  message order so token numbering is deterministic across the resends of one
  conversation.
*/
